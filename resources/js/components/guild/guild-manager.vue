<template>
    <div
        class="fixed inset-0 p-4 flex flex-wrap justify-center items-center w-full h-full z-[1000] before:fixed before:inset-0 before:w-full before:h-full before:bg-[rgba(0,0,0,0.8)] overflow-auto font-[sans-serif]">
        <div @click.stop class="w-full max-w-md bg-bars shadow-lg rounded-lg p-6 relative">
            <button class="float-right" @click="cancelAction">
                <font-awesome-icon :icon="faX()" size="1x"/>
            </button>
            <div class="my-8 text-center">
                <h4 class="text-white/80 text-lg font-semibold">Register New Guild</h4>
                <form ref="form" @submit.prevent="submit">
                    <div class="flex flex-col gap-4 mt-5">
                        <input
                            v-model="data.name"
                            type="text"
                            placeholder="Name"
                            required
                            class="p-3 rounded-lg text-black">
                        <input
                            v-model="data.icon_url"
                            type="url"
                            placeholder="Icon url"
                            class="p-3 rounded-lg text-black">
                        <textarea
                            v-model="data.description"
                            type="text"
                            placeholder="Description"
                            class="p-3 rounded-lg text-black">
                        </textarea>
                        <button
                            type="submit"
                            class="p-3 bg-blue-500 rounded-lg w-1/2 mx-auto"
                        >
                            Confirm
                        </button>
                    </div>
                </form>
            </div>
        </div>
    </div>
</template>

<script>
import {FontAwesomeIcon} from "@fortawesome/vue-fontawesome";
import { faX} from "@fortawesome/free-solid-svg-icons";

export default {
    data() {
        return {
            data: {
                name: '',
                description: '',
                icon_url: '',
            }
        }
    },
    props: {
        cancelAction: {
            type: Function,
            required: true
        },
    },
    components: {FontAwesomeIcon},
    methods: {
        faX() {
            return faX
        },
        async submit() {
            const valid = this.$refs.form.checkValidity();
            if (!valid) return;

            if (!this.data.icon_url)
                this.data.icon_url = 'https://via.placeholder.com/640x480.png/000?text=' + this.data.name.charAt(0).toUpperCase();

            if (this.data.description === '')
                delete this.data.description;

            this.cancelAction();

            await this.$store.dispatch('guilds/store', this.data);
        },
    },
}
</script>
